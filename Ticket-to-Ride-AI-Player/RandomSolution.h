...
public:
	...
	if ((ret==1)||(ret==0))
	{
	  ...
	  */
	bool attemptToDrawFromBlindDeck(void);
	bool attemptToDrawFromOpenDeck(void);
	void drawCards(void);
	...
	if (ret>=0) {
	  ...
	  */
	bool attemptToClaimColoredRoute(TTRCard *currentCard, int nWildCards);
	bool attemptToClaimNeutralRoute(TTRCard *currentCard, int nWildCards);
